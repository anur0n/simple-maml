...
from utils import weighted_normalize
...
class BatchEpisodes(object):
    ...
    def observations(self):
        if self._observations is None:
            ...
            observations = np.zeros(
                (len(self), self.batch_size) + observation_shape, dtype=np.float64
            )
    ...
    def actions(self):
        if self._actions is None:
            ...
            actions = np.zeros(
                (len(self), self.batch_size) + action_shape, dtype=np.float64
            )
    ...
    def rewards(self):
        if self._rewards is None:
            rewards = np.zeros((len(self), self.batch_size), dtype=np.float64)
    ...
    def returns(self):
        if self._returns is None:
            ...
            return_ = torch.zeros((self.batch_size,), dtype=torch.float64)
    ...
    def mask(self):
        if self._mask is None:
            self._mask = torch.zeros(
                (len(self), self.batch_size), dtype=torch.float64, device=self.device
            )
    ...
    def append(self, observations, actions, rewards, batch_ids):
        for observation, action, reward, batch_id in zip(
            ...
            if batch_id is None:
                continue
            self._observations_list[batch_id].append(observation.astype(np.float64))
            self._actions_list[batch_id].append(action.astype(np.float64))
            self._rewards_list[batch_id].append(reward.astype(np.float64))
    ...
    def compute_advantages(self, baseline, gae_lambda=1.0, normalize=True):
        ...
        gae = torch.zeros((self.batch_size,), dtype=torch.float64)
        ...
        # Once the advantages are computed, the returns are not necessary anymore (only to compute the parameters of the baseline)
        del self._returns
        del self._mask
